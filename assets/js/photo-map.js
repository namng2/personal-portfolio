...
//
// Provider: Leaflet + Esri's World Topographic basemap. No API key, which is
// the constraint that rules out most alternatives on a public repo — a key in
// the source is a key anyone can spend. Topo rather than the Gray Canvas
// because it is in colour: soft land, green parks, blue water, close to the
// Apple Maps palette. Labels are baked in, so it is a single layer.
//
// Note the tile path is {z}/{y}/{x}, not Leaflet's usual {z}/{x}/{y}.
//
// Dark mode filters the tiles rather than swapping to a dark basemap: Esri has
// no colour dark twin, and the grey one throws the colour away. The inversion
// plus a 180deg hue rotation darkens the land while leaving water blue and
// parks green, and it flips the baked-in labels to light-on-dark for free.
//
// Two providers were tried and rejected, both worth remembering:
//   - CARTO's basemaps now serve an "API KEY REQUIRED" watermark as HTTP 200
//     image/png. Check the pixels, not the status code.
//   - OpenStreetMap's own tiles work in a browser but return a 403 "Access
//     blocked" image to command-line clients, so curl lies about them in both
//     directions. They also carry a usage policy this site should not lean on.
//
...
(function photoMap() {
  ...
  const ATTRIB =
    'Tiles &copy; <a href="https://www.esri.com" target="_blank" rel="noopener">Esri</a> and contributors';
  const TILE_OPTS = { attribution: ATTRIB, maxNativeZoom: 19, maxZoom: 19 };
  ...
  function build() {
    ...
    baseLayer = L.tileLayer(TILES, TILE_OPTS).addTo(map);
    applyMode();
    ...
    // Watched at the DOM level rather than through Leaflet's dragstart/zoomstart,
    // because fitBounds fires those itself — the first programmatic fit would
    // otherwise mark the map as user-moved and block every later re-fit.
  ...
  // One basemap, filtered for dark. Cheaper than swapping URLs — the tiles
  // stay cached across a theme change instead of being refetched.
  function applyMode() {
    host.classList.toggle("pm-map-dark", currentMode() === "dark");
  }
  window.addEventListener("themechange", () => {
    if (baseLayer) applyMode();
  });
  ...
  // fitBounds is only meaningful once the container has its real size. The
  // panel starts display:none and the window is resizable, so the first fit
  // routinely runs against a mis-measured box and lands the view somewhere
  // else entirely. Re-fit whenever the size changes, until the visitor moves
  // the map themselves — after that the view is theirs, not ours.
  function fitToPhotos() {
    if (userMoved || !map || !photoBounds.length) return;
    if (photoBounds.length === 1) map.setView(photoBounds[0], 13);
    else map.fitBounds(photoBounds, { padding: [50, 50], maxZoom: 14 });
  }
